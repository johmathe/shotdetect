...
DialogShotDetect::DialogShotDetect(wxWindow *parent, int id,
                                   const wxString &title, const wxPoint &pos,
                                   const wxSize &size, long style)
    : wxDialog(parent, id, title, pos, size, wxDEFAULT_DIALOG_STYLE) {
  // SetIcon (wxICON (icone_cgp));

  list_films = new wxListCtrl(this, -1, wxDefaultPosition, wxDefaultSize,
  ...
  label_id_film = new wxStaticText(this, -1, wxT("Movie ID"), wxDefaultPosition,
                                   wxDefaultSize, wxALIGN_RIGHT);
  text_id_film = new wxTextCtrl(this, -1, wxT("movie"));
  label_id_auteur =
      new wxStaticText(this, -1, wxT("author id"), wxDefaultPosition,
                       wxDefaultSize, wxALIGN_RIGHT);
  ...
  LabelSeuil = new wxStaticText(this, -1, wxT("threshold"), wxDefaultPosition,
  ...
  text_resume =
      new wxTextCtrl(this, -1, wxT("Abstract"), wxPoint(180, 90),
                     wxSize(200, 70), wxTE_MULTILINE | wxTE_PROCESS_TAB);
  ...
  const wxString radio_box_1_choices[] = {wxT("Iri"), wxT("Autres")};
  radio_box_1 =
      new wxRadioBox(this, -1, wxT("Format"), wxDefaultPosition, wxDefaultSize,
                     2, radio_box_1_choices, 0, wxRA_SPECIFY_COLS);
  set_properties();
  do_layout();
...
EVT_MENU(ID_CMD_LAUNCHTHREAD, DialogShotDetect::FinProcess)
    EVT_BUTTON(ID_AJOUTER, DialogShotDetect::AjouterFichier)
    EVT_BUTTON(ID_ENLEVER, DialogShotDetect::EnleverFichier)
    EVT_BUTTON(ID_VIDER, DialogShotDetect::ViderListe)
    EVT_BUTTON(ID_PARCOURIR, DialogShotDetect::ParcourirSortie)
    EVT_BUTTON(ID_PARCOURIR_XSL, DialogShotDetect::ParcourirXsl)
    EVT_BUTTON(ID_LANCER, DialogShotDetect::ProcessVideo)
    EVT_BUTTON(ID_HELP, DialogShotDetect::Help)
    EVT_CLOSE(DialogShotDetect::OnQuit)
    // end wxGlade
    END_EVENT_TABLE();
...
void DialogShotDetect::AjouterFichier(wxCommandEvent &event) {
  ...
  if (dialog.ShowModal() == wxID_OK) {
    long id =
        list_films->InsertItem(list_films->GetItemCount(), dialog.GetPath(), 0);
...
void DialogShotDetect::EnleverFichier(wxCommandEvent &event) {
  long item = -1;
  for (;;) {
    item =
        list_films->GetNextItem(item, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
    if (item == -1) break;
...
void DialogShotDetect::ViderListe(wxCommandEvent &event) {
...
void DialogShotDetect::Help(wxCommandEvent &event) {
  hframe->ShowModal();
  event.Skip();
}

void DialogShotDetect::ParcourirSortie(wxCommandEvent &event) {
...
void DialogShotDetect::ParcourirXsl(wxCommandEvent &event) {
  wxFileDialog dialog(this, _T ("Open an xsl stylesheet"), wxEmptyString,
                      wxEmptyString,
                      _T ("Xslt files (*.xsl;*.xslt)|*.xsl;*.xslt"));
  dialog.SetDirectory(wxGetHomeDir());
  if (dialog.ShowModal() == wxID_OK) {
...
void DialogShotDetect::set_properties() {
...
void DialogShotDetect::do_layout() {
...
void DialogShotDetect::set_progress_local(double percent) {
  wxString str;
  str << int(percent);
  str << wxT(".00 %");
...
void DialogShotDetect::set_progress_global(double val_global) {
  wxString str;
  str << int(val_global);
  str << wxT(".00 %");
...
void DialogShotDetect::set_time_elapsed(double time) {
  wxString str;
...
int DialogShotDetect::get_time_elapsed() {
